...
func (gw *GinWeb) HandleBridgeFilesUpload(c *gin.Context) {
	fmt.Println("HandleBridgeFilesUpload - ")
	...
	if err != nil {
		fmt.Println("HandleBridgeFilesUpload - err ", err.Error())
		c.String(http.StatusBadRequest, fmt.Sprintf("File Form err: %s", err.Error()))
		c.JSON(http.StatusOK, result.Set(bzweb.ErrCode_error, err.Error()))
		return
	}
	fmt.Println("HandleBridgeFilesUpload -1 ", form.File["files"])
	upload := &FileUpload{
	...
	c.JSON(http.StatusOK, result.Set(bzweb.ErrCode_success, fmt.Sprintf("Uploaded successfully %d files with save to path<%s>.", len(files), upload.FilePath)))
}
